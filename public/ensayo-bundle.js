...
class DualStreamEngine {
  ...
  _getActiveVoicePool() {
    if (this.traste_groups.size === 0) {
      return this.selectedVoiceIds ? this.canteVoices.map((v, i) => ({
        v,
        i
      })).filter(({
        v
  ...
  _startPalmasLoop() {
    ...
    this.palmasShifter = new PitchShifter(this.audioContext, this.palmasBuffer, 4096, () => {
      if (!this.isPlaying) return;
...
class EnsayoMode {
  ...
  onCommand(cb) {
    this.onCommandListeners.push(cb);
  }
  onVoiceStatus(cb) {
    this.onVoiceStatusListeners.push(cb);
  }
  _notifyCommand(cmd) {
...
const resolveFetch$3 = customFetch => {
  ...
  if (customFetch) {
    ...
    _fetch = (...args) => Promise.resolve().then(function () { return browser; }).then(({
      default: fetch
    }) => fetch(...args));
  } else {
    _fetch = fetch;
...
function head(fetcher, url, options, parameters) {
  return __awaiter$5(this, void 0, void 0, function* () {
    return _handleRequest$1(fetcher, 'HEAD', url, Object.assign(Object.assign({}, options), {
      noResolveJson: true
    }), parameters);
  });
}
...
function _generateLinkResponse(data) {
  const {
    ...
    rest = __rest$1(data, ["action_link", "email_otp", "hashed_token", "redirect_to", "verification_type"]);
  const properties = {
    action_link,
    email_otp,
    hashed_token,
...
const supabaseUrl = "https://bjyfaqgwdqvgcjhssfhj.supabase.co";
const supabaseAnonKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJqeWZhcWd3ZHF2Z2NqaHNzZmhqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk2OTcxMDUsImV4cCI6MjA5NTI3MzEwNX0.WRuxRfZf0Yo_EZe_1uh2NKycrjk9oXJQt6JegVKltAU";
...
const canteTracksAPI = {
  ...
  async getTracksByPalo(palo) {
    const {
      data,
      error
    } = await supabase.from('cante_tracks').select('*').eq('palo', palo).order('created_at', {
      ascending: true
    });
    if (error) {
  ...
  async addTrack(track) {
    const {
      data,
      error
    } = await supabase.from('cante_tracks').insert([track]).select().single();
    if (error) {
  ...
  async getAllTracks() {
    const {
      data,
      error
    } = await supabase.from('cante_tracks').select('*').order('palo', {
      ascending: true
    }).order('created_at', {
      ascending: true
    });
  ...
  async deleteTrack(trackId, audioUrl) {
    ...
    const {
      error
    } = await supabase.from('cante_tracks').delete().eq('id', trackId);
    if (error) {
...
const ensayoAPI = {
  async getPalmasBaseByPalo(palo) {
    const {
      data,
      error
    } = await supabase.from('palmas_bases').select('*').eq('palo', palo).order('created_at', {
      ascending: true
    }).limit(1).maybeSingle();
    if (error) throw error;
    return data;
  ...
  async getCanteVoicesByPalo(palo) {
    const {
      data,
      error
    } = await supabase.from('cante_voices').select('*').eq('palo', palo).order('created_at', {
      ascending: true
    });
    if (error) throw error;
    return data || [];
  ...
  async getAllPalmasBases() {
    const {
      data,
      error
    } = await supabase.from('palmas_bases').select('*').order('palo', {
      ascending: true
    });
    if (error) throw error;
    return data || [];
  ...
  async getAllCanteVoices() {
    const {
      data,
      error
    } = await supabase.from('cante_voices').select('*').order('palo', {
      ascending: true
    }).order('created_at', {
      ascending: true
    });
  ...
  async uploadAndCreatePalmasBase(file, palo, title, bpm, beatsPerCompas) {
    ...
    const {
      data,
      error
    } = await supabase.from('palmas_bases').insert([{
      palo,
      title,
  ...
  async uploadAndCreateCanteVoice(file, palo, title) {
    ...
    const {
      data,
      error
    } = await supabase.from('cante_voices').insert([{
      palo,
      title,
      audio_url: urlData.publicUrl
    }]).select().single();
  ...
  async deletePalmasBase(id, audioUrl) {
    ...
    const {
      error
    } = await supabase.from('palmas_bases').delete().eq('id', id);
    if (error) throw error;
  },
...
class EnsayoApp {
  ...
  _recordCanteEntry(voice) {
    ...
    this._canteLog.push({
      title: voice.canonical_title || voice.title,
...
//# sourceMappingURL=ensayo-bundle.js.map
